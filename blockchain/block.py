...
class Block:
    ...
    def mine_block(last_block, data):
        ...
        while hex_to_binary(hash)[0:difficulty] != '0' * difficulty:
            nonce += 1
            timestamp = time.time_ns()
            # adjust the new timestamp for new hash creating
            difficulty = Block.adjust_difficulty(last_block, timestamp)
            # same generates the new difficulty
            hash = crypto_hash(timestamp, last_hash, data, difficulty, nonce)
            # creates the new hash with new difficulty and new timestamp
...
def main():
    ...
    block_2 = Block.mine_block(genesis_block, 55)
    Block.mine_block(block_2, 99)
    print(Block.is_valid_block(genesis_block, block_2))             # check verify the valid block or not
